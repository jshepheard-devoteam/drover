...
Drover does not add a dedicated `docker()` option for every Docker run feature (custom `context`, `ports` publishing, and similar one-off flags).
...
- **`dockerCompose()`** (where available) delegates container configuration to a user-managed `docker-compose.yml`. Image, networks, published ports, GPU reservations, resource limits, and dependent services all live in the compose file; Drover injects only the per-run worktree mount, workdir, and env. Every Docker option is reachable this way today.
