...
import * as drover from "./SandboxProvider.js";
...
describe("printFileDisplayStartup", () => {
  ...
  it("does not use clack (no @clack/prompts calls)", async () => {
    ...
    const clackSpy = vi
      ...
      .mockImplementation(() => {});
    printFileDisplayStartup({
      logPath: "/project/.drover/logs/main.log",
    });
  ...
  it("shows branch name when provided", () => {
    printFileDisplayStartup({
      logPath: "/project/.drover/logs/main.log",
      branch: "drover/issue-124-file-logging",
    });
    const allOutput = consoleSpy.mock.calls.flat().join(" ");
    expect(allOutput).toContain("drover/issue-124-file-logging");
  });
  ...
  it("prints a relative log path when hostRepoDir equals process.cwd()", () => {
    const logPath = join(process.cwd(), ".drover", "logs", "main.log");
    printFileDisplayStartup({
      logPath,
      hostRepoDir: process.cwd(),
    });
    const allOutput = consoleSpy.mock.calls.flat().join(" ");
    expect(allOutput).toContain("tail -f .drover/logs/main.log");
  ...
  it("prints an absolute log path when hostRepoDir differs from process.cwd()", () => {
    ...
    const logPath = join(hostRepoDir, ".drover", "logs", "main.log");
    printFileDisplayStartup({
      logPath,
      hostRepoDir,
    });
    ...
    expect(allOutput).toContain(
      "tail -f /some/other/repo/.drover/logs/main.log",
    );
  });
});
...
describe("RunResult", () => {
  it("includes logFilePath when logging to a file", () => {
    const result: RunResult = {
      ...
      commits: [],
      branch: "main",
      logFilePath: "/path/to/drover.log",
    };
    expect(result.logFilePath).toBe("/path/to/drover.log");
  });
...
describe("branchStrategy on RunOptions", () => {
  it("throws when head strategy is used with an isolated provider", async () => {
    const isolatedSandbox = drover.createIsolatedSandboxProvider({
      name: "test-isolated",
      create: async () => ({
...
describe("sanitizeBranchForFilename", () => {
  ...
  it("replaces forward slashes with dashes", () => {
    expect(sanitizeBranchForFilename("drover/issue-87-log-file")).toBe(
      "drover-issue-87-log-file",
    );
  });
  ...
  it("handles nested slashes like a typical drover branch", () => {
    expect(
      sanitizeBranchForFilename("drover/issue-87-log-file-branch-name"),
    ).toBe("drover-issue-87-log-file-branch-name");
  });
});

describe("defaultImageName", () => {
  it("returns drover:<dir-name> for a typical repo path", () => {
    expect(defaultImageName("/home/user/my-project")).toBe("drover:my-project");
  });

  it("lowercases the directory name", () => {
    expect(defaultImageName("/home/user/MyProject")).toBe("drover:myproject");
  });
  ...
  it("handles paths with trailing slash gracefully", () => {
    expect(defaultImageName("/home/user/my-repo/")).toBe("drover:my-repo");
  });
});
...
describe("buildLogFilename", () => {
  ...
  it("prefixes with target branch when temp branch is used", () => {
    expect(buildLogFilename("drover/20260325-142719", "main")).toBe(
      "main-drover-20260325-142719.log",
    );
  });

  it("sanitizes target branch with slashes", () => {
    expect(buildLogFilename("drover/20260325-142719", "feature/my-work")).toBe(
      "feature-my-work-drover-20260325-142719.log",
    );
  });

  it("includes agent name when branch contains agent segment", () => {
    expect(buildLogFilename("drover/claude-code/20260325-142719", "main")).toBe(
      "main-drover-claude-code-20260325-142719.log",
    );
  });
  ...
  it("appends run name after target branch prefix", () => {
    expect(buildLogFilename("drover/20260325-142719", "main", "reviewer")).toBe(
      "main-drover-20260325-142719-reviewer.log",
    );
  });
...
describe("promptFile resolution with cwd", () => {
  ...
  it("resolves relative promptFile from process.cwd(), not from cwd", async () => {
    ...
    const cwdDir = mkdtempSync(join(tmpdir(), "drover-cwd-"));
...
describe("run() error logging to file", () => {
  ...
  it("writes SandboxError to log file when using file logging", async () => {
    const dir = mkdtempSync(join(tmpdir(), "drover-run-error-"));
...
describe("structured output entry-time validation", () => {
  ...
  it("validates tag presence with promptFile", async () => {
    const dir = mkdtempSync(join(tmpdir(), "drover-output-"));
    const promptFile = join(dir, "prompt.md");
...
describe("output.maxRetries end-to-end", () => {
  ...
  it("retries once on bad JSON, succeeds on resumed second attempt", async () => {
    const hostProjectsDir = mkdtempSync(join(tmpdir(), "drover-retry-"));
