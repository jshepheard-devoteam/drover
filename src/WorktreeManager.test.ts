 ...
 * simulate someone else moving origin forward between drover runs.
 */
...
describe("generateTempBranchName", () => {
  it("returns a string in drover/<YYYYMMDD-HHMMSS>-<random> format", () => {
    const name = generateTempBranchName();
    expect(name).toMatch(/^drover\/\d{8}-\d{6}-[0-9a-f]{6}$/);
  });
  ...
  it("includes sanitized name when provided", () => {
    ...
    expect(name).toMatch(/^drover\/my-run\/\d{8}-\d{6}-[0-9a-f]{6}$/);
  });
...
describe("WorktreeManager.create", () => {
  it("creates a worktree at .drover/worktrees/<name>/", async () => {
    const repoDir = await setupRepo();
    const { path } = await run(create(repoDir));
    expect(path).toContain(join(repoDir, ".drover", "worktrees"));
  ...
  it("creates a drover/<timestamp>-<random> branch when no branch is specified", async () => {
    const repoDir = await setupRepo();
    const { branch } = await run(create(repoDir));
    expect(branch).toMatch(/^drover\/\d{8}-\d{6}-[0-9a-f]{6}$/);
  });

  it("includes name in branch when name is specified", async () => {
    const repoDir = await setupRepo();
    const { branch } = await run(create(repoDir, { name: "my-run" }));
    expect(branch).toMatch(/^drover\/my-run\/\d{8}-\d{6}-[0-9a-f]{6}$/);
  });

  it("includes name in worktree directory when name is specified", async () => {
    const repoDir = await setupRepo();
    const { path } = await run(create(repoDir, { name: "my-run" }));
    expect(path).toMatch(/drover-my-run-\d{8}-\d{6}-[0-9a-f]{6}$/);
  });
  ...
  it("the worktree directory is on the correct branch", async () => {
    ...
    expect(stdout.trim()).toMatch(/^drover\//);
  });
  ...
  it("creates a new branch from HEAD when specified branch does not exist", async () => {
    const repoDir = await setupRepo();
    const { path, branch } = await run(
      create(repoDir, { branch: "drover/issue-42-new-feature" }),
    );

    expect(branch).toBe("drover/issue-42-new-feature");
    expect(await getBranch(path)).toBe("drover/issue-42-new-feature");
  ...
  it("detects collision when branch is checked out in the main working tree", async () => {
    ...
    // The error should explain why this happens (drover uses worktrees,
  ...
  it("does not write upstream tracking config even when autoSetupMerge is enabled", async () => {
    ...
    const { path, branch } = await run(
      create(repoDir, { branch: "drover/no-tracking-test" }),
    );
    ...
    const { stdout: trackingConfig } = await execAsync(
      `git config --get-regexp "branch\\.drover/no-tracking-test\\." || true`,
      { cwd: repoDir },
    );
...
describe("WorktreeManager.pruneStale", () => {
  ...
  it("removes orphaned directories under .drover/worktrees/", async () => {
    const repoDir = await setupRepo();
  ...
  it("does not remove active worktrees when .drover is a symlink", async () => {
    // Regression test for #470: git canonicalizes worktree paths, so when
    // .drover is a symlink the un-canonicalized entryPath never matched
    ...
    await symlink(externalDir, join(repoDir, ".drover"));
